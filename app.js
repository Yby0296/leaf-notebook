...
(function () {
  ...
  function load() {
    ...
    // 兼容旧数据：预定计划补上模块标签
    try { (data.plans.items || []).forEach(p => { if (!p.module) p.module = "general"; }); } catch (e) {}
    rolloverCheck();
    save();
  }
  function save() {
  ...
  function addDaysKey(key, n) { const d = parseKey(key); d.setDate(d.getDate() + n); return ymd(d); }
  ...
  const MODULES = [
    ...
    { k: "exercise", n: "运动计划", i: "🏃" },
    { k: "video", n: "视频剪辑", i: "🎬" },
    { k: "english", n: "英语口语", i: "🗣️" },
    { k: "account", n: "记账理财", i: "💰" },
    { k: "review", n: "每日复盘", i: "📝" },
    { k: "general", n: "其他安排", i: "🌿" },
  ];
  function moduleMeta(k) { return MODULES.find(m => m.k === k) || MODULES[MODULES.length - 1]; }

  /* 通用「预约到某天」卡片：各模块复用 */
  function bookCard(mod, placeholder) {
    const tm = tomorrowKey();
    return `
      <div class="card book-card">
        <div class="card-h"><div class="t"><span class="dot dot-blue"></span>⏰ 提前预约</div><span class="rate-pill" style="background:var(--blue);color:#3d6474">明天 ${tm.slice(5)}</span></div>
        <div class="module-sub" style="margin:0 0 8px">预约后会在当天的「每日任务」里置顶提醒</div>
        <div class="add-form">
          <input class="f-name" id="bk_title" placeholder="${esc(placeholder)}" />
          <input class="f-sel" id="bk_date" type="date" value="${tm}" />
          <input class="f-sel" id="bk_time" type="time" />
          <button class="btn" data-action="book-module" data-module="${mod}">+ 预约</button>
        </div>
        <div class="quick-days">
          <span class="tag" data-action="book-quick" data-day="0">今天</span>
          <span class="tag" data-action="book-quick" data-day="1">明天</span>
          <span class="tag" data-action="book-quick" data-day="2">后天</span>
          <span class="tag" data-action="book-quick" data-day="7">下周</span>
        </div>
      </div>`;
  }
  /* 创建一条带模块标签的预定 */
  function createPlan(mod, title, date, time, note) {
    data.plans.items.push({ id: uid(), title, date, time: time || "", note: note || "", module: mod || "general", done: false });
    save();
    toast(date === todayKey() ? "🔔 已加入今日任务" : "📅 已预约 " + date + (time ? " " + time : "") + "：" + title);
  }
  ...
  /* ---------------- 打卡核心 ---------------- */
  function ensureDay(kind, d) {
    d = d || todayKey();
    if (!data[kind].days[d]) {
      if (kind === "learning") data[kind].days[d] = { big: [], small: [] };
      else data[kind].days[d] = [];
    }
    return data[kind].days[d];
  }
  // 学习任务
  function learnDone(sub, id, d) {
    d = d || todayKey();
    if (data.settings.mode === "daily") return (data.learning.days[d]?.[sub] || []).includes(id);
    return !!data.learning.templates[sub].find(t => t.id === id)?.done;
  }
  function toggleLearn(sub, id, d) {
    d = d || todayKey();
    const tpl = data.learning.templates[sub].find(t => t.id === id);
    if (!tpl) return;
    if (data.settings.mode === "daily") {
      const arr = ensureDay("learning", d)[sub];
  ...
  // 运动
  function exerDone(id, d) {
    d = d || todayKey();
    if (data.settings.mode === "daily") return (data.exercise.days[d] || []).includes(id);
    return !!data.exercise.templates.find(t => t.id === id)?.done;
  }
  function toggleExer(id, d) {
    d = d || todayKey();
    const tpl = data.exercise.templates.find(t => t.id === id);
    if (!tpl) return;
    if (data.settings.mode === "daily") {
      const arr = ensureDay("exercise", d); const i = arr.indexOf(id);
      if (i >= 0) arr.splice(i, 1); else arr.push(id);
    } else {
      tpl.done = !tpl.done;
      const arr = ensureDay("exercise", d); const i = arr.indexOf(id);
  ...
  function render() {
    ...
    // 内容（顶部拼接今日提醒横幅）
    const map = { learning: renderLearning, exercise: renderExercise, video: renderVideo, english: renderEnglish, account: renderAccount, review: renderReview, plans: renderPlans, daily: renderDaily };
    const html = (map[UI.module] || renderLearning)();
    content.innerHTML = (UI.module === "daily" ? "" : reminderBanner()) + html;
  }
  ...
  function reminderBanner() {
    ...
    ["big", "small"].forEach(sub => data.learning.templates[sub].forEach(tp => { lTotal++; if (learnDone(sub, tp.id, t)) lDone++; }));
    if (lDone < lTotal) inc.push("学习 " + (lTotal - lDone));
    // 运动
    let eTotal = 0, eDone = 0;
    data.exercise.templates.forEach(tp => { eTotal++; if (exerDone(tp.id, t)) eDone++; });
    if (eDone < eTotal) inc.push("运动 " + (eTotal - eDone));
    // 预定
    const pDue = (data.plans.items || []).filter(p => p.date === t && !p.done).length;
    if (pDue) inc.push("预约 " + pDue);
    const tmrN = (data.plans.items || []).filter(p => p.date === tomorrowKey()).length;
    const tmrTip = tmrN ? `<div class="reminder-banner" style="background:var(--blue)"><span class="bell">🌙</span>明天已预约 ${tmrN} 项，安排得明明白白~</div>` : "";
    if (!inc.length) return tmrTip;
    return `<div class="reminder-banner"><span class="bell">🌿</span>今日还有 ${inc.join(" · ")} 项待完成，去「每日任务」打卡吧~</div>` + tmrTip;
  }
    ...
    return `
      <div class="card">
        ...
        <div class="add-form">
          ...
          <input class="f-sel" id="ld_${sub}" type="date" placeholder="日期（可选）" title="留空=每天固定任务；选日期=只预约那天" />
          <button class="btn" data-action="add-learning" data-sub="${sub}">+ 添加</button>
        </div>
        <div class="quick-days">
          <span class="hint-txt">日期留空 = 每天固定任务</span>
          <span class="tag" data-action="learn-quick" data-sub="${sub}" data-day="1">预约明天</span>
          <span class="tag" data-action="learn-quick" data-sub="${sub}" data-day="2">后天</span>
          <span class="tag" data-action="learn-quick" data-sub="${sub}" data-day="-1">清空</span>
        </div>
    ...
    return `
      ...
      <div class="card">
        ...
        <div class="add-form">
          ...
          <input class="f-sel" id="ex_date" type="date" placeholder="日期（可选）" title="留空=每天固定项目；选日期=只预约那天" />
          <button class="btn" data-action="add-exercise">+ 添加项目</button>
        </div>
        <div class="quick-days">
          <span class="hint-txt">日期留空 = 每天固定项目</span>
          <span class="tag" data-action="exer-quick" data-day="1">预约明天</span>
          <span class="tag" data-action="exer-quick" data-day="2">后天</span>
          <span class="tag" data-action="exer-quick" data-day="-1">清空</span>
        </div>
        <div class="stat-row" style="margin-top:12px">
  ...
  function renderVideo() {
    ...
    return `
      ...
      ${bookCard("video", "明天要学的剪辑技巧，如：学转场卡点")}
      <div class="tags">
  ...
  function renderEnglish() {
    ...
    return `
      ...
      ${bookCard("english", "明天的口语目标，如：练机场值机对话")}
      <div class="card">
        <div class="dialogue">
  ...
  function renderAccount() {
    ...
    return `
      ...
      ${bookCard("account", "明天的财务安排，如：交房租 / 只花50元")}
  ...
  function renderPlans() {
    ...
    const list = items.map(p => {
      ...
      const mm = moduleMeta(p.module);
      return `
      <div class="item ${p.done ? "done" : ""}">
        <div class="check" data-action="toggle-plan" data-id="${p.id}">${checkSvg()}</div>
        <div class="body">
          <div class="name"><span class="mod-chip">${mm.i} ${mm.n}</span>${esc(p.title)}</div>
    ...
    return `
      ...
      <div class="module-sub">提前预约未来安排（可选模块 + 具体时间），到当天会在「每日任务」对应板块置顶提醒</div>
      <div class="card">
        ...
        <div class="add-form">
          ...
          <select class="f-sel" id="pl_module">
            ${MODULES.map(m => `<option value="${m.k}"${m.k === "general" ? " selected" : ""}>${m.i} ${m.n}</option>`).join("")}
          </select>
          <input class="f-sel" id="pl_date" type="date" value="${tomorrowKey()}" />
        ...
        <div class="quick-days">
          <span class="tag" data-action="plan-quick" data-day="0">今天</span>
          <span class="tag" data-action="plan-quick" data-day="1">明天</span>
          <span class="tag" data-action="plan-quick" data-day="2">后天</span>
          <span class="tag" data-action="plan-quick" data-day="7">下周</span>
        </div>
      </div>
      <div class="card">
  ...
  /* ===================================================================
     8. 每日任务（汇总各模块当日计划，统一打卡 + 提醒）
     =================================================================== */
  function renderDaily() {
    const d = UI.dailyDate || todayKey();
    const isToday = d === todayKey();

    // 当天的预定计划，按模块归类
    const dayPlans = (data.plans.items || []).filter(p => p.date === d);
    const planOf = k => dayPlans.filter(p => (p.module || "general") === k)
      .sort((a, b) => (a.time || "99").localeCompare(b.time || "99"))
      .map(p => ({ kind: "plan", id: p.id, name: p.title, time: p.time, note: p.note, booked: true, done: !!p.done }));

    // 各板块：预定项置顶 + 日常模板
    const groups = [];

    const lTpl = [];
    ["big", "small"].forEach(sub => data.learning.templates[sub].forEach(tp => {
      lTpl.push({ kind: "learning", id: tp.id, sub, name: tp.name, done: learnDone(sub, tp.id, d) });
    }));
    groups.push({ k: "learning", items: planOf("learning").concat(lTpl) });

    const eTpl = data.exercise.templates.map(tp => ({ kind: "exercise", id: tp.id, name: tp.name, done: exerDone(tp.id, d) }));
    groups.push({ k: "exercise", items: planOf("exercise").concat(eTpl) });

    ["video", "english", "account", "review", "general"].forEach(k => groups.push({ k, items: planOf(k) }));

    let total = 0, done = 0;
    groups.forEach(g => { total += g.items.length; done += g.items.filter(x => x.done).length; });

    const sec = (g) => {
      if (!g.items.length) return "";
      const mm = moduleMeta(g.k);
      const doneN = g.items.filter(x => x.done).length;
      const rows = g.items.map(it => `
        <div class="item ${it.done ? "done" : ""}${it.booked ? " booked" : ""}">
          <div class="check" data-action="daily-toggle" data-kind="${it.kind}" data-sub="${it.sub || ""}" data-id="${it.id}">${checkSvg()}</div>
          <div class="body">
            <div class="name">${it.booked ? '<span class="mod-chip book">预约</span>' : ""}${esc(it.name)}</div>
            ${it.time ? `<div class="meta">🕐 ${esc(it.time)}${it.note ? " · " + esc(it.note) : ""}</div>` : (it.note ? `<div class="meta">${esc(it.note)}</div>` : "")}
          </div>
        </div>`).join("");
      return `
        <div class="card">
          <div class="card-h"><div class="t"><span class="dot dot-pink"></span>${mm.i} ${mm.n}</div><span class="rate-pill">${doneN}/${g.items.length}</span></div>
          ${rows}
        </div>`;
    };
    ...
    let banner = "";
    if (isToday) {
      if (done < total) banner = `<div class="reminder-banner"><span class="bell">🌿</span>今日还有 ${total - done} 项任务待完成，加油哦~</div>`;
      else if (total > 0) banner = `<div class="reminder-banner" style="background:var(--blue-deep)"><span class="bell">✅</span>今日任务已全部完成，太棒啦！</div>`;
    } else {
      const bk = dayPlans.length;
      if (bk) banner = `<div class="reminder-banner" style="background:var(--blue)"><span class="bell">📌</span>${d === tomorrowKey() ? "明天" : d} 已预约 ${bk} 项，到当天会自动提醒</div>`;
    }

    // 明天预约速览
    const tmr = (data.plans.items || []).filter(p => p.date === tomorrowKey());
    const tmrTip = (isToday && tmr.length) ? `<div class="card"><div class="card-h"><div class="t"><span class="dot dot-blue"></span>🌙 明天已预约</div><span class="rate-pill" style="background:var(--blue);color:#3d6474">${tmr.length} 项</span></div>${tmr.map(p => `<div class="item"><div class="body"><div class="name"><span class="mod-chip">${moduleMeta(p.module).i} ${moduleMeta(p.module).n}</span>${esc(p.title)}</div>${p.time ? `<div class="meta">🕐 ${esc(p.time)}</div>` : ""}</div></div>`).join("")}</div>` : "";

    return `
      <div class="module-title">📋 每日任务</div>
      <div class="module-sub">把当天各模块的计划汇总在一起，统一打卡；预约的事项会置顶到对应板块</div>
      <div class="card">
        <div class="card-h"><div class="t">📅 选择日期</div><span class="rate-pill">${done}/${total}</span></div>
        <div class="add-form">
          <input class="f-sel" id="daily_date" type="date" value="${d}" style="flex:1 1 100%" />
        </div>
        <div class="quick-days">
          <span class="tag ${isToday ? "active" : ""}" data-action="daily-quick" data-day="0">今天</span>
          <span class="tag ${d === tomorrowKey() ? "active" : ""}" data-action="daily-quick" data-day="1">明天</span>
          <span class="tag ${d === addDaysKey(todayKey(), 2) ? "active" : ""}" data-action="daily-quick" data-day="2">后天</span>
        </div>
        ${!isToday ? `<div class="module-sub" style="margin:6px 0 0">查看 ${d}（${weekdayCn(parseKey(d))}）的任务</div>` : ""}
      </div>
      ${banner}
      ${groups.map(sec).join("")}
      ${total === 0 ? '<div class="empty"><span class="em">📭</span>这天还没有任何任务，去各模块预约吧</div>' : ""}
      ${tmrTip}
    `;
  }
  ...
  function handleAction(a, el) {
    switch (a) {
      ...
      case "add-learning": {
        ...
        const bd = $("#ld_" + sub) ? $("#ld_" + sub).value : "";
        if (!name) { toast("先写点任务说明吧~"); return; }
        if (bd) { createPlan("learning", name + (est ? "（" + est + "分钟）" : ""), bd, "", ""); render(); break; }
        data.learning.templates[sub].push({ id: uid(), name, est, done: false });
        save(); render(); break;
      }
      case "learn-quick": {
        const n = parseInt(el.dataset.day, 10);
        const inp = $("#ld_" + el.dataset.sub); if (inp) inp.value = n < 0 ? "" : addDaysKey(todayKey(), n);
        break;
      }
      ...
      case "add-exercise": {
        ...
        const bd = $("#ex_date") ? $("#ex_date").value : "";
        if (!name) { toast("先填运动项目~"); return; }
        if (bd) { createPlan("exercise", name + (target ? "（" + target + "分钟" + (count > 1 ? "×" + count + "组" : "") + "）" : ""), bd, "", ""); render(); break; }
        data.exercise.templates.push({ id: uid(), name, target, count, done: false }); save(); render(); break;
      }
      case "exer-quick": {
        const n = parseInt(el.dataset.day, 10);
        const inp = $("#ex_date"); if (inp) inp.value = n < 0 ? "" : addDaysKey(todayKey(), n);
        break;
      }
      ...
      /* 每日任务 */
      case "daily-quick": {
        UI.dailyDate = addDaysKey(todayKey(), parseInt(el.dataset.day, 10) || 0); render(); break;
      }
      case "daily-toggle": {
        const kind = el.dataset.kind;
        const view = UI.dailyDate || todayKey();
        if (kind === "learning") toggleLearn(el.dataset.sub, el.dataset.id, view);
        else if (kind === "exercise") toggleExer(el.dataset.id, view);
        else if (kind === "plan") {
          const p = data.plans.items.find(x => x.id === el.dataset.id);
      ...
      case "add-plan": {
        ...
        const mod = $("#pl_module") ? $("#pl_module").value : "general";
        if (!title || !date) { toast("请填写内容并选择日期~"); return; }
        createPlan(mod, title, date, time, note);
        render(); break;
      }
      case "plan-quick": {
        const n = parseInt(el.dataset.day, 10) || 0;
        const inp = $("#pl_date"); if (inp) inp.value = addDaysKey(todayKey(), n);
        break;
      }
      /* 各模块「提前预约」 */
      case "book-module": {
        const mod = el.dataset.module;
        const title = ($("#bk_title") || {}).value ? $("#bk_title").value.trim() : "";
        const date = ($("#bk_date") || {}).value || "";
        const time = ($("#bk_time") || {}).value || "";
        if (!title) { toast("先写下要做什么吧~"); return; }
        if (!date) { toast("请选择日期~"); return; }
        createPlan(mod, title, date, time, "");
        render(); break;
      }
      case "book-quick": {
        const n = parseInt(el.dataset.day, 10) || 0;
        const inp = $("#bk_date"); if (inp) inp.value = addDaysKey(todayKey(), n);
        break;
      }
      case "toggle-plan": {
      ...
      case "edit-plan": {
        ...
        const mod = prompt("所属模块（learning/exercise/video/english/account/review/general）", p.module || "general");
        if (mod !== null && MODULES.some(m => m.k === mod.trim())) p.module = mod.trim();
  ...
  document.addEventListener("change", (e) => {
    ...
    if (e.target.id === "daily_date") { UI.dailyDate = e.target.value || todayKey(); render(); }
  });
