...
(function () {
  ...
  function reminderBanner() {
    ...
    if (pDue) inc.push("预定 " + pDue);
    if (!inc.length) return "";
    return `<div class="reminder-banner"><span class="bell">🌿</span>今日还有 ${inc.join(" · ")} 项待完成，去「每日任务」打卡吧~</div>`;
  }
  ...
  function renderDaily() {
    ...
    // 学习任务（big + small 模板即每日计划）
    const lItems = [];
    ["big", "small"].forEach(sub => data.learning.templates[sub].forEach(tp => {
      lItems.push({ id: tp.id, sub, name: tp.name, done: learnDone(sub, tp.id, d) });
    }));
    const lDone = lItems.filter(x => x.done).length;

    // 运动计划（模板即每日计划）
    const eItems = data.exercise.templates.map(tp => ({ id: tp.id, name: tp.name, done: exerDone(tp.id, d) }));
    const eDone = eItems.filter(x => x.done).length;

    // 预定计划（日期命中 d）
    const pItems = (data.plans.items || []).filter(p => p.date === d)
      .map(p => ({ id: p.id, name: p.title, time: p.time, note: p.note, done: !!p.done }));
    const pDone = pItems.filter(x => x.done).length;

    const total = lItems.length + eItems.length + pItems.length;
    const done = lDone + eDone + pDone;

    const sec = (title, icon, items, kind, doneN) => {
      if (!items.length) return "";
      const rows = items.map(it => `
        <div class="item ${it.done ? "done" : ""}">
          <div class="check" data-action="daily-toggle" data-kind="${kind}" data-sub="${it.sub || ""}" data-id="${it.id}">${checkSvg()}</div>
          <div class="body">
      ...
      return `
        <div class="card">
          <div class="card-h"><div class="t"><span class="dot dot-pink"></span>${icon} ${title}</div><span class="rate-pill">${doneN}/${items.length}</span></div>
          ${rows}
        </div>`;
    };
    ...
    return `
      ...
      <div class="module-sub">把当天各模块的计划汇总在一起，统一打卡</div>
      <div class="card">
      ...
      ${banner}
      ${sec("学习任务", "📚", lItems, "learning", lDone)}
      ${sec("运动计划", "🏃", eItems, "exercise", eDone)}
      ${sec("预定计划", "📅", pItems, "plan", pDone)}
      ${total === 0 ? '<div class="empty"><span class="em">📭</span>这天还没有任何任务，去各模块添加吧</div>' : ""}
    `;
  }
